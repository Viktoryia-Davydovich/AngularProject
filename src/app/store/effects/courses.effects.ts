...
import { listCourses, CoursesActions, getCourselist, onFoundCourseById } from '../actions/courses.actions';
import { Observable } from 'rxjs';

@Injectable()
export class CoursesEffects {
  constructor(private actions$: Actions<CoursesActions>, private courseService: CourseService, private store: Store<IAppState>) {}
      ...
      this.actions$.pipe(
        ...
        exhaustMap(action =>
          this.courseService.createCourse(action.course).pipe(
            map(() => getCourselist({start: 0, end: 3}))
          )
        )
      );
        ...
        updateCourse$ = () =>
          ...
          findCourses$ = () =>
            this.actions$.pipe(
              ...
              exhaustMap(action =>
                this.courseService.searchCourses(action.searchString).pipe(
                  map(() => getCourselist({start: 0, end: 3}))
                )
              )
            );
              ...
              this.actions$.pipe(
                ...
                exhaustMap(action =>
                  this.courseService.getCourseById(action.id).pipe(
                    map((foundCourse: Course) => onFoundCourseById({foundCourseById: foundCourse}))
                  )
                )
              );
}
